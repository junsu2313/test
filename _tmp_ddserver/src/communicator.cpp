...
namespace {
uint32_t readBulkSizeFromEnvironment()
{
	const char *raw = getenv("DDSERVER_BULK_READ_SIZE");
	unsigned long value = raw ? strtoul(raw, nullptr, 10) : 32768UL;
	if (value < 512UL)
		value = 512UL;
	if (value > 262144UL)
		value = 262144UL;
...
bool readFlagFromEnvironment(const char *name)
{
	const char *raw = getenv(name);
	return raw != nullptr && (strcmp(raw, "1") == 0 || strcmp(raw, "true") == 0);
}
...
Communicator::Communicator() : mSocket(0), mCtx(nullptr), //mIsInitialized(false), mIsUsbInitialized(false),
							   ...
							   mBulkReadSize(readBulkSizeFromEnvironment()),
							   mTraceUsb(readFlagFromEnvironment("DDSERVER_TRACE_USB")),
							   mBulkReadCalls(0), mUsbReadStartedAtUs(0), mUsbFirstByteAtUs(0)
...
void Communicator::handleClientConnection(int socket)
{
	...
	int no_delay = 1;
	if (setsockopt(mSocket, IPPROTO_TCP, TCP_NODELAY, &no_delay, sizeof(no_delay)) < 0)
		syslog(LOG_ERR, "TCP_NODELAY setsockopt failed");
	...
	while(true) {
		if (!readFromClient()) {
			syslog(LOG_INFO, "Stopping client");
			break;
		}
	}
...
bool Communicator::readSocketExact(uint8_t *buf, uint32_t length)
{
	if (buf == nullptr)
		return false;
	...
	uint32_t offset = 0;
	while (offset < length) {
		ssize_t r = read(mSocket, buf + offset, length - offset);
		if (r > 0) {
		...
		if (r == 0) {
			if (offset == 0)
				syslog(LOG_INFO, "Client closed socket");
			else
				syslog(LOG_ERR, "Socket closed while reading %u bytes, received %u", length, offset);
			return false;
		}
		if (errno == EINTR)
			continue;

		syslog(LOG_ERR, "Socket read error while reading %u bytes at offset %u: %d", length, offset, errno);
		return false;
	}
...
bool Communicator::readFromClient()
{
	...
	if (!readSocketExact(reinterpret_cast<uint8_t *>(&packetSize), 4))
		return false;

	packetSize = le32toh(packetSize);
	if (packetSize < 4) {
		syslog(LOG_ERR, "Invalid packet size %u", packetSize);
		return false;
	}

	uint32_t payloadSize = packetSize - 4;
	uint8_t *buf = (uint8_t *) malloc(payloadSize);
	if (buf == nullptr) {
		syslog(LOG_ERR, "Unable to allocate packet buffer for %u bytes", payloadSize);
		return false;
	}

	if (payloadSize == 0 || readSocketExact(buf, payloadSize))
		result = processPacket(buf, payloadSize);
	else
		syslog(LOG_ERR, "Error reading packet payload of %u bytes", payloadSize);
...
int Communicator::sendBuffer(uint8_t *buf, int size)
{
	int sent = 0;
	while (sent < size) {
		ssize_t r = write(mSocket, buf + sent, size - sent);
		if (r > 0) {
		...
		if (r < 0 && errno == EINTR)
			continue;
		syslog(LOG_ERR, "Error sending packet to client at %d/%d: %d", sent, size, errno);
		break;
	}
	return sent;
}
...
{
	if (buf == nullptr || size < PTP_HEADER) {
		syslog(LOG_ERR, "Invalid client PTP packet size %d", size);
		return false;
	}
...
bool Communicator::processUsbPacket(uint8_t * buf, int size)
{
	bool result = false;
	if (buf == nullptr || size < PTP_HEADER) {
		syslog(LOG_ERR, "USB command packet is too short: %d", size);
		return false;
	}
	PtpPacket *header = (PtpPacket *)&buf[0];
	int packetSize = le32toh(header->packet_len);
	if (packetSize < PTP_HEADER || packetSize > size) {
		syslog(LOG_ERR, "Invalid USB command packet size %d in payload %d", packetSize, size);
		return false;
	}
	int writen = 0;
	const uint16_t commandCode = le16toh(header->packet_command);
	const bool isLiveViewFrame = commandCode == 0x9203;
	const uint64_t requestStartedAtUs = isLiveViewFrame ? monotonicUs() : 0;
	if (isLiveViewFrame) {
		mBulkReadCalls = 0;
		mUsbReadStartedAtUs = 0;
		mUsbFirstByteAtUs = 0;
	}

	// is Nikon set application mode
	bool isNikon = mVendorId == 0x04b0 && commandCode == 0x1016 && packetSize >= 16 && *(uint32_t *)&buf[12] == htole32(0xd1f0);
	if (isNikon)
...
//			syslog(LOG_INFO, "sending PTP data packet");
			...
			const int dataPacketSize = le32toh(header->packet_len);
			if (dataPacketSize < PTP_HEADER || dataPacketSize > size - packetSize) {
				syslog(LOG_ERR, "Invalid USB data packet size %d in remaining payload %d", dataPacketSize, size - packetSize);
				return false;
			}
			r = libusb_bulk_transfer(mHandle, mWriteEndpoint, &buf[packetSize], dataPacketSize, &writen, 4000);
			if (r == 0) {
				if (writen != dataPacketSize)
					syslog(LOG_ERR, "Command data Packet size was: %d  writen: %d", dataPacketSize, writen);
...
*/
	...
	if (isLiveViewFrame && mTraceUsb) {
		const uint64_t doneAtUs = monotonicUs();
		const uint64_t firstByteUs = mUsbFirstByteAtUs > requestStartedAtUs
			? mUsbFirstByteAtUs - requestStartedAtUs : 0;
		const uint64_t totalUs = doneAtUs > requestStartedAtUs
			? doneAtUs - requestStartedAtUs : 0;
		syslog(LOG_INFO,
			"liveview9203 bulk_size=%u bulk_reads=%u first_byte_us=%llu total_us=%llu",
			mBulkReadSize,
			mBulkReadCalls,
			static_cast<unsigned long long>(firstByteUs),
...
{
	static const uint32_t MAX_PACKET_SIZE = 64U * 1024U * 1024U;
    ...
    int runNo = 0;
	if (mUsbReadBuffer.size() < static_cast<size_t>(currentPacketSize))
		mUsbReadBuffer.resize(currentPacketSize);
	uint8_t *buf = mUsbReadBuffer.data();
    length = 0;
	...
	while(packetSize1 == 0 || totalRead < packetSize1) {
        ...
        if (readPtpPacket(&buf[offset], currentPacketSize - offset, readBytes, isInterrupt, timeOut)) {
            ...
            if (totalRead >= 4) {
				...
				if (packetSize1 < PTP_HEADER || packetSize1 > MAX_PACKET_SIZE) {
					syslog(LOG_ERR, "Invalid USB response packet size %u", packetSize1);
					return nullptr;
				}
                if (packetSize1 > currentPacketSize - 4) {
                    currentPacketSize = 4 + packetSize1 + 4096;
                    mUsbReadBuffer.resize(currentPacketSize);
                    buf = mUsbReadBuffer.data();
					// resize() may move the backing storage; refresh every pointer
					// derived from the vector before inspecting the packet again.
					ptpPacket = (PtpPacket *)&buf[4];
                }
            }
        }
            ...
            while(packetSize2 == 0 || totalRead < packetSize2) {
                if (readPtpPacket(&buf[offset], currentPacketSize - offset - 4, readBytes, isInterrupt, timeOut)) {
					...
					if (totalRead >= 4) {
						...
						if (packetSize2 < PTP_HEADER || packetSize2 > 4096) {
							syslog(LOG_ERR, "Invalid USB trailing response size %u", packetSize2);
							return nullptr;
						}
					}
                }
...
bool Communicator::readPtpPacket(uint8_t *buf, int bufSize, int &length, bool interrupt, unsigned int timeOut )
{
    ...
    int transferSize = bufSize < static_cast<int>(mBulkReadSize)
		? bufSize : static_cast<int>(mBulkReadSize);
    unsigned int tOut = 10000;
    if (interrupt) {
        ep = mInterruptEndpoint;
        transferSize = mInterruptMax < bufSize ? mInterruptMax : bufSize;
	if (timeOut != 0)
	        tOut = timeOut;
	else
		tOut = 5;
...
//                r = libusb_bulk_transfer(mHandle, ep, buf, bufSize, &readBytes, tOut);
		if (interrupt) {
			 r = libusb_interrupt_transfer(mHandle, ep, buf, transferSize, &readBytes, tOut);
			if (r == 0) {
			 ...
			 if (mUsbReadStartedAtUs == 0)
				 mUsbReadStartedAtUs = monotonicUs();
			 mBulkReadCalls++;
			 r = libusb_bulk_transfer(mHandle, ep, buf, transferSize, &readBytes, tOut);
		}
		if (r == 0) {
			if (readBytes > 0) {
				if (!interrupt && mUsbFirstByteAtUs == 0)
					mUsbFirstByteAtUs = monotonicUs();
				success = true;
				length = readBytes;
				break;
			} else {
					...
					while (i < config->bNumInterfaces) {
						...
						while (j < inter->num_altsetting) {
							...
							if (interdesc->bInterfaceClass == 6) {
								...
								result = claimInterface(readEp, readMax, writeEp, writeMax, interruptEp, interruptMax, interdesc->bInterfaceNumber);
								if (result)
								{
...
bool Communicator::claimInterface(uint8_t readEp, uint16_t readMax, uint8_t writeEp, uint16_t writeMax, uint8_t interruptEp, uint16_t interruptMax, int interfaceNo)
{
	...
	if (libusb_kernel_driver_active(mHandle, mImagingInterface) == 1) { //find out if kernel driver is attached
		syslog(LOG_INFO, "Kernel driver active, trying to detach");
		if (libusb_detach_kernel_driver(mHandle, mImagingInterface) == 0) //detach it
				...
				while (j < inter->num_altsetting) {
					...
					if (interdesc->bInterfaceClass == 6) {
						...
						if (r != 0) {
							...
							if (libusb_kernel_driver_active(deviceHandle, interdesc->bInterfaceNumber) == 1) { //find out if kernel driver is attached
								if (libusb_detach_kernel_driver(deviceHandle, interdesc->bInterfaceNumber) == 0)
									syslog(LOG_INFO, "Kernel driver detached");
								else
									syslog(LOG_INFO, "Kernel driver detach failed");
							}

							if( libusb_claim_interface(deviceHandle, interdesc->bInterfaceNumber) == 0) { //claim imaging interface
								libusb_release_interface(deviceHandle, interdesc->bInterfaceNumber);
								libusb_close(deviceHandle);
								result = true;
								break;
							}
							else
								syslog(LOG_INFO, "Claim interface failed");
						}
						break;
					}
					j++;
			...
			if (interdesc->bInterfaceClass == 6) {
				...
				if (r != 0) {
					...
					break;
							...
							if (libusb_kernel_driver_active(deviceHandle, interdesc->bInterfaceNumber) == 0) { //find out if kernel driver is attached
