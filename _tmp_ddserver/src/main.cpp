...
int main() {
	...
	const char *disableUdp = getenv("DDSERVER_DISABLE_UDP");
	bool udpEnabled = !(disableUdp && strcmp(disableUdp, "1") == 0);
	if (udpEnabled) {
		pthread_t myThread;
		...
		syslog(LOG_INFO, "UDP listener disabled by DDSERVER_DISABLE_UDP");
	}
