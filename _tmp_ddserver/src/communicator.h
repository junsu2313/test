...
private:
	...
	uint16_t mVendorId;
	uint16_t mProductId;
	uint32_t mBulkReadSize;
	bool mTraceUsb;
	uint32_t mBulkReadCalls;
	uint64_t mUsbReadStartedAtUs;
	uint64_t mUsbFirstByteAtUs;
	std::vector<uint8_t> mUsbReadBuffer;
	...
	bool readFromClient();
	bool readSocketExact(uint8_t *buf, uint32_t length);
